...
- section을 돌면서 현재 section (next)에서 롤러를 칠해가면서 롤러를 칠한 마지막 번호를 저장해놓고 (current), 현재 section (next)이 마지막 번호(current) 보다 크면 다시 롤러를 칠하는 방식으로 계산한다.

```kotlin
fun solution(n: Int, m: Int, section: IntArray): Int {
    var answer = 0
    var current = 0
    section.forEach { next ->
        if (next > current) {
            answer++
            current = next + m - 1
        }
