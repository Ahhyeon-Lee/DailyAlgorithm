...
fun solution(progresses: IntArray, speeds: IntArray): IntArray {
    val queue: Queue<Int> = LinkedList<Int>().apply {
        addAll(
            progresses.mapIndexed { index, value ->
                ceil((100-value) / speeds[index].toDouble()).toInt()
            }
        )
    }
    ...
    while (queue.isNotEmpty()) {
        val next = queue.indexOfFirst { queue.peek() < it }
        answer = answer.plus(next.takeIf { it != -1 } ?: queue.size)
        repeat(next.takeIf { it != -1 } ?: queue.size) {
            queue.poll()
        }
    }
