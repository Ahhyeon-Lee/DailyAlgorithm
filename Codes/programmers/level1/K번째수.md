...
fun solution(array: IntArray, commands: Array<IntArray>): IntArray {
    ...
    commands.forEachIndexed { index, value ->
        val sliced = array.slice(value[0]-1 until value[1])
        answer[index] = (sliced.sorted()[value[2]-1])
    }
...
- 사이즈가 정해진 IntArray에 값을 넣어서 푼 방법.

```kotlin
fun solution(array: IntArray, commands: Array<IntArray>): IntArray {
    return commands.map {
        val sliced = array.slice(it[0]-1 until it[1])
        sliced.sorted()[it[2]-1]
    }.toIntArray()
}
```

- 어차피 정답 IntArray의 size는 commands의 size만큼이므로 map으로 값을 변환하여 리턴한 방법.
